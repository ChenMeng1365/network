...
```ruby
@sign << ["T8000-18", "接口配置"]
@sign << ["T8000-18", "接口物理信息"]
@sign << ["T8000-18", "接口基本信息"]
@sign << ["T8000-18", "接口vlan信息"]
@sign << ["T8000-18", "接口acl信息"]
@sign << ["T8000-18", "接口urpf信息"]
@sign << ["T8000-18", "接口mpls信息"]
@sign << ["T8000-18", "接口lacp信息"]
@sign << ["T8000-18", "接口am信息"]
@sign << ["T8000-18", "接口uim信息"]
@sign << ["T8000-18", "接口isis信息"]
@sign << ["T8000-18", "接口multicast信息"]
@sign << ["T8000-18", "接口QoS"]
@sign << ["T8000-18", "接口流策略"]
@sign << ["T8000-18", "接口统计"]
@sign << ["T8000-18", "接口URPF"]
@sign << ["T8000-18", "接口接入类型"]
@sign << ["T8000-18", "特殊接口配置"]
@sign << ["T8000-18", "物理接口配置"]
@sign << ["T8000-18", "子接口配置"]
@sign << ["T8000-18", "接口分捡"]
@sign << ["T8000-18", "端口识别"]
@sign << ["T8000-18", "端口MAC"]
@sign << ["T8000-18", "端口管理状态"]
@sign << ["T8000-18", "接口描述"]
@sign << ["T8000-18", "接口地址"]
@sign << ["T8000-18", "接口开关"]
@sign << ["T8000-18", "接口vpn实例"]
@sign << ["T8000-18", "聚合接口判定"]
@sign << ["T8000-18", "聚合接口分捡"]
@sign << ["T8000-18", "聚合逻辑接口"]
@sign << ["T8000-18", "聚合接口查询"]
@sign << ["T8000-18", "vlan分捡"]
@sign << ["T8000-18", "vlan统计"]
@sign << ["T8000-18", "端口去0"]
@sign << ["T8000-18", "统计接口"]
...
module T8000_18
  ...
  # T8000的接口信息获取麻烦得多，需要针对各类配置模块获取接口相关的信息，所以全局配置都要解析一次，目前要解析的清单如下：
  # 1. 接口物理信息
  # 2. 接口基本信息
  # 3. 接口vlan信息
  # 4. 接口acl信息
  ...
  def 端口识别 描述
    ...
    连接符 = '-'
    type = /xgei|gei|smartgroup|loopback|eth_dslgroup|extimer|vbui|vlan|null|guvi|gcvi|gps|gre_tunnel|mgmt_eth|spi|ptp|qx_eth|qx/.match(描述)
    类型 = type ? type.to_s : '未知类型'
    port = /(\d+|\/|\.)*(\d+)/.match(描述.split(类型).join)
    端口 = port ? port.to_s : '未知端口'
    连接符 = '' if ['smartgroup','loopback','eth_dslgroup','vbui','vlan','null','qx','gre_tunnel'].include?(类型)
    端口 = '' if 类型=='mgmt_eth'
    return 类型,连接符,端口
  end
