...
@sign << ["M6000-8", "接口物理信息"]
@sign << ["M6000-8", "接口基本信息"]
@sign << ["M6000-8", "接口vlan信息"]
@sign << ["M6000-8", "接口acl信息"]
@sign << ["M6000-8", "接口urpf信息"]
@sign << ["M6000-8", "接口mpls信息"]
@sign << ["M6000-8", "接口lacp信息"]
@sign << ["M6000-8", "接口am信息"]
@sign << ["M6000-8", "接口uim信息"]
@sign << ["M6000-8", "接口isis信息"]
@sign << ["M6000-8", "接口multicast信息"]
@sign << ["M6000-8", "接口QoS"]
@sign << ["M6000-8", "接口流策略"]
@sign << ["M6000-8", "接口统计"]
@sign << ["M6000-8", "接口URPF"]
...
module M6000_8
  ...
  def 端口识别 描述
    ...
    连接符 = '-'
    type = /xgei|gei|smartgroup|loopback|eth_dslgroup|extimer|vbui|supervlan|vlan|virtual_template|null|guvi|gcvi|gre_tunnel|mgmt_eth|qx_eth|qx|spi/.match(描述)
    类型 = type ? type.to_s : '未知类型'
    port = /(\d+|\/|\.)*(\d+)/.match(描述.split(类型).join)
    端口 = port ? port.to_s : '未知端口'
    连接符 = '' if ['smartgroup','loopback','eth_dslgroup','gre_tunnel','vbui','supervlan','vlan','virtual_template','null','qx'].include?(类型)
    端口 = '' if 类型=='mgmt_eth'
    return 类型,连接符,端口
  end
