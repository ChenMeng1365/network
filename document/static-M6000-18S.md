...
```ruby
@sign << ['M6000-18S', '静态用户地址']
@sign << ['M6000-18S', '静态接口地址']
@sign << ['M6000-18S', '静态域地址']
...
module M6000_18S
  ...
  def 静态域地址 raw,*guards
    ...
    textlist = content.split('INTERFACE(Exvlan/Invlan)             CreateTime          ')[1].to_s.split('--------------------------------------------------------------------------------')[0].to_s
    textlist.split("\n").each do|line|
      item = line.strip.split(' ')
      if item.size == 3 && item[1].include?('.')
        item.insert 1, ''
        tab << item
      elsif item.size == 3 && !item[1].include?('.')
        tab[-1] += [item[0], item[1..2].join(' ')]
      end
    end
    return tab
  end
